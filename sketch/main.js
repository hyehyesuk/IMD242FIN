...
let vibrationAmplitude = 0; // 진동 크기를 저장하는 변수
...
function setup() {
  ...
  slider = createSlider(0, 20, 0.5, 0.01);
  song.setVolume(0.5);
  ...
  slider.position(windowWidth * 0.5 - 100, windowHeight * 0.2);
}
...
function draw() {
  ...
  // 슬라이더 값에 따라 진동 크기 업데이트
  vibrationAmplitude = slider.value();

  const bgColor = map(vibrationAmplitude, 0, 50, 0, 255); // 배경색은 진동 크기에 따라 변경
  ...
  stack.bodies.forEach((aBody) => {
    // 각 오브젝트에 진동 추가
    const offsetX = random(-vibrationAmplitude, vibrationAmplitude);
    const offsetY = random(-vibrationAmplitude, vibrationAmplitude);
    Body.translate(aBody, { x: offsetX, y: offsetY });
